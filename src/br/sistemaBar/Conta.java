...
public class Conta {
	...
	public double calcularSomaDosItens(){
		
		double valorTotal = 0;
		
		for(Pedido p : this.pedidos){
			for(ItemDePedido i : p.getItens()){
				valorTotal += (i.getQuantidade() * i.getValorUnidade());
			}
		}
		return valorTotal;
	}
	
	public double calcularPorcGarcom(){
		
		return (this.calcularSomaDosItens() + this.calcularTotalCouvert()) * (this.porGarcom/100);
	}
	
	public double calcularTotalCouvert(){
		
		return this.quantPessoas * this.valorCouvert;
	}
	
	public double calcularTotal(){
		
		return this.calcularSomaDosItens() + this.calcularTotalCouvert() + this.calcularPorcGarcom() - this.valorDesconto;
	}
	
	
	public double calcularTroco(){
		
		return this.valorRecebido - this.calcularParcial();		
	}
	...
	public void fecharMesa(int numMesa){
		//TODO
	}
